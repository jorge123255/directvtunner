...
class DirectvEpg {
  ...
  startAutoRefresh() {
    ...
    console.log(`[epg] Auto-refresh enabled (every ${AUTO_REFRESH_INTERVAL / 1000 / 60 / 60} hours)`);
    ...
    this.refreshTimer = setInterval(() => this.autoRefresh(), AUTO_REFRESH_INTERVAL);
  }
  ...
  async fetchFromBrowser() {
    ...
    try {
      ...
      if (apiResponses.channels?.channelInfoList) {
        this.channels = apiResponses.channels.channelInfoList.map(ch => ({
          id: ch.resourceId,
          name: ch.channelName,
  ...
  generateM3U(host) {
    ...
    for (const channel of this.channels) {
      ...
      m3u += `http://${host}/tve/directv/stream/${channel.number}\n\n`;
    }
